...
static float xIncrement = 0; // x movement on vaxman
static float yIncrement = 0; // y movement on vaxman
static int rotation = 0; // orientation of vaxman
...
bool foodEaten(int x, int y, float vaxmanX, float vaxmanY) {
	if (x >= vaxmanX - 16.0 * cos(359 * M_PI / 180.0) && x <= vaxmanX + 16.0 * cos(359 * M_PI / 180.0)) {
		if (y >= vaxmanY - 16.0 * cos(359 * M_PI / 180.0) && y <= vaxmanY + 16.0 * cos(359 * M_PI / 180.0)) {
			return true;
		}
	}
	return false;
...
void drawFood(float vaxmanX, float vaxmanY) {
	deque<float> temp;
	//check if the food has not been eaten
	for (int i = 0; i < food.size(); i = i + 2) {
		if (!foodEaten(food.at(i) * squareSize, food.at(i + 1) * squareSize, vaxmanX, vaxmanY)) {
...
void gameOver() {
	int vaxmanX = (int)(1.5 + xIncrement);
	int vaxmanY = (int)(1.5 + yIncrement);
	...
	for (auto& v : v_all_ghosts) {					// Grab each set of instances
		...
		for (auto& ghost : v) {					// for every ghost in each set
			if (vaxmanX == (int)(ghost[0]) &&	// vaxmanX == ghostX
				vaxmanY == (int)(ghost[1])) {	// vaxmanY == ghostY
...
void welcomeScreen() {
	...
	while (*message)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++);
	message = "vaxman - Original By Patricia Terol";
	glColor3f(1, 1, 1);
	...
	while (*message)
		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, *message++);
	message = "To control vaxman use A to go right, D to go left, W to go up and S to go down.";
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("vaxman - original by Patricia Terol\nAdapted by Jibran Absarulislam and\nNixon Puertollano");
